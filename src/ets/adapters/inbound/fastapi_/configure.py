...
from ets.adapters.inbound.fastapi_.routes import router


def get_openapi_schema(app: FastAPI, config: ApiConfigBase) -> dict[str, Any]:
    """Generate a custom OpenAPI schema for the service."""
    return get_openapi(
        title="EM Transformation Service",
        version=__version__,
        description="A minimal mock service passing the workflow id to application layer",
