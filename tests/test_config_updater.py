...
def _make_updater(
    *,
    ...
    config_manager = MagicMock(spec=ConfigManagerPort)
    config_manager.resolve_and_persist = AsyncMock(side_effect=resolve_raises)

    versioner = MagicMock(spec=ConfigVersionerPort)
    versioner.get_version = AsyncMock(side_effect=[previous_version, current_version])
    ...
    return updater, config_lock, config_manager, versioner, incoming_aem_pack_queue


@pytest.mark.asyncio()
async def test_lock_acquired_version_changed_marks_for_reprocessing():
    """When the persisted config changes during resolve, the queue is flagged."""
    updater, lock, manager, versioner, queue = _make_updater(
        lock_acquired=True, previous_version=3, current_version=4
    )
    ...
    assert versioner.get_version.await_count == 2
...
@pytest.mark.asyncio()
async def test_lock_acquired_version_unchanged_does_not_reprocess():
    """When the persisted config is unchanged, the queue is not touched."""
    updater, lock, manager, versioner, queue = _make_updater(
        lock_acquired=True, previous_version=7, current_version=7
    )
...
async def test_lock_not_acquired_waits_for_release():
    ...
    updater, lock, manager, versioner, queue = _make_updater(
        lock_acquired=False, previous_version=0, current_version=0
    )
    ...
    versioner.get_version.assert_not_awaited()
    queue.mark_all_for_reprocessing.assert_not_awaited()
...
async def test_resolve_failure_releases_lock_and_propagates():
    ...
    updater, lock, manager, _, queue = _make_updater(
        lock_acquired=True,
        previous_version=1,
        current_version=1,
        resolve_raises=boom,
