...
"""Tests for the pruning logic in ConfigUpdater."""
...
async def test_resolve_and_persist(
    ...
    monkeypatch.setattr(config_updater_module, "compare_configs", compare_configs)
...
async def test_resolve_and_persist_stops_when_no_persisted_config(
    ...
    monkeypatch.setattr(
        config_updater_module, "compare_configs", MagicMock(return_value=raw_config)
    )
    monkeypatch.setattr(
