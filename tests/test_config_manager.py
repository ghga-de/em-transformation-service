...
"""Unit tests for the ConfigManager startup orchestrator."""
...
def _make_manager(
    *,
    ...
    """Build a ConfigManager wired with port mocks tuned for one branch."""
    ...
    config_updater = MagicMock(spec=ConfigUpdater)
    if resolve_raises is not None:
        config_updater.resolve_and_persist = AsyncMock(side_effect=resolve_raises)
    else:
        config_updater.resolve_and_persist = AsyncMock(return_value=config_has_changed)
    ...
    return manager, config_lock, config_updater, incoming_aem_pack_queue
...
async def test_lock_acquired_config_changed_marks_for_reprocessing():
    ...
    manager, lock, updater, queue = _make_manager(
...
async def test_lock_not_acquired_waits_for_release():
    ...
    manager, lock, updater, queue = _make_manager(lock_acquired=False)
